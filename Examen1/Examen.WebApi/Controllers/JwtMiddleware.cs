...
namespace Lec10.Application
{
    public class JwtMiddleware
    {
        ...
        public static bool TryGetToken(HttpRequest request, out string token) {
            token = null;
            HttpRequestMessage requestMessage = new HttpRequestMessageFeature(request.HttpContext).HttpRequestMessage;

            IEnumerable<string> authorizationHeaders;
            if (!requestMessage.Headers.TryGetValues("Authorization", out authorizationHeaders) || authorizationHeaders.Count() > 1) {
                return false;
            }
            var bearer = authorizationHeaders.FirstOrDefault();
            token = bearer.StartsWith("Bearer")
            ? bearer.Substring(7)
            : bearer;
        ...
        public async Task Invoke(HttpContext context) {
            bool success = context.GetEndpoint().Metadata.OfType<AllowAnonymousAttribute>().Any();
